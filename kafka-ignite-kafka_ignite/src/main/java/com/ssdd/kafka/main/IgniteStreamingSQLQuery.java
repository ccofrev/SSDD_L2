...
public class IgniteStreamingSQLQuery implements Runnable {
    ...
    private static final List<String> QUERIES = Arrays.asList(
            "SELECT deviceId, city, time, AVG(temperature) AS AvgTemp FROM WeatherSignalEntity " + "GROUP BY deviceId ORDER BY time DESC LIMIT 5",
            "SELECT deviceId, city, time, AVG(feelsLike) AS AvgFeelsLike FROM WeatherSignalEntity " + "GROUP BY city ORDER BY time DESC LIMIT 5",
            "SELECT deviceId, city, time, AVG(pressure) AS AvgPress FROM WeatherSignalEntity " + "GROUP BY deviceId ORDER BY time DESC LIMIT 5");
